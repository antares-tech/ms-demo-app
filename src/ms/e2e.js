...
export function isSupported()
{
	if (e2eSupported === undefined)
	{
		if (RTCRtpSender.prototype.createEncodedStreams)
		{
			...
			{
				log.debug(`isSupported() | not supported: ${error}`);
		...
		{
			log.debug('isSupported() | not supported');
...
{
	log.debug('setupSenderTransform()');
...
{
	log.debug('setupReceiverTransform()');
